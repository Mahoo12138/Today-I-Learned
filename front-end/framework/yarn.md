...
Yarn Workspaces（工作空间/工作区，本文使用工作空间这一名称）是 Yarn 提供的`Monorepo`依赖管理机制，从 Yarn 1.0 开始默认支持，用于在代码仓库的**根目录**下管理**多个 project**的依赖。
Yarn Workspaces 的目标是使使用 *Monorepo* 变得简单，以一种更具声明性的方式处理`yarn link`的主要使用场景。
...
代价是即使只开发其中一个 package 也需要安装整个项目的依赖。以 _jest_ 为例，其 Monorepo 代码结构为：
...
> 注意：`private: true`配置是必需的！工作区并不意味着要被发布，所以需要添加了这个安全措施，以确保不会发布到 npm 仓库。

- 使用`yarn workspaces info [--json]`命令可以获得整个 workspace 的目录结构；
