...
> Max-Age 属性指定从现在开始 Cookie 存在的秒数，比如 60 _60_ 24 \* 365（即一年）。过了这个时间以后，浏览器就不再保留这个 Cookie。  
...
- Redis（推荐）：内存型数据库，正合 sessionId-sessionData 的场景，且访问快；
...
前面的图很清楚了，服务端要实现对 cookie 和 session 的存取，实现起来要做的事还是很多的。在`npm`中，已经有封装好的中间件，比如 `express-session`，用法就不贴了。

`express-session` 主要实现了：
...
那我们如何控制 token 的有效期呢？很简单，把「过期时间」和用户数据一起塞进去，验证时判断就好。
...
比如 node 端的 cookie-session  库：

> 不要纠结名字，其实是个 token 库，但保持了和 express-session 高度一致的用法，把要存的数据挂在 session 上

默认配置下，当我给他一个 userid，他会存成这样：

| Name  | Value                | Domain    | Path | Expires | HTTP | Secure |
| ----- | -------------------- | --------- | ---- | ------- | ---- | ------ |
| token | eyJ1c2VyaWQiOiJhIn0= | localhost | /    | Nerver  | true | false  |
...
解决方案就是给 token 加签名，来识别 token 是否被篡改过。例如在 cookie-session 库中，增加两项配置：
...
这样会多种一个 token.sig 的 cookie 字段，里面的值就是 `{"userid":"abb”}` 和 `iAmSecret`通过加密算法计算出来的，常见的比如 HMACSHA256 类 。

好了，现在用户 cdd 虽然能伪造出 `eyJ1c2VyaWQiOiJhIn0=`，但伪造不出 sig 的内容，因为他不知道 secret 密钥。

### JWT

但上面的做法额外增加了 cookie 数量，数据本身也没有规范的格式，所以 [JSON Web Token](http://jwt.io) 横空出世了。
...
![JWT](jwt.png)
类型、加密算法的选项，以及 JWT 标准数据字段，可以参考 [RFC 7519: JSON Web Token (JWT)](https://www.rfc-editor.org/rfc/rfc7519)。

node 上同样有相关的库实现：express-jwt，koa-jwt。
