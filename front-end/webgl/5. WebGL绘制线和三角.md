...
回顾之前我们绘制点的做法，我们都是通过 `gl.drawArrays()` 来实现单个点的绘制，而且当时我们还简单介绍了一下这个 api 的用法（[[2. WebGL绘制点]]）。我们在这里简单回顾一下：
...
其实这一步我们并不陌生，回顾[[front-end/webgl/3. WebGL绘制动态点|3. WebGL绘制动态点]]，之前我们是通过 `gl.vertexAttrib2f` 将数据动态传递到 `a_Position` 中的，然后再赋值给 `gl_Position`。但是这个 api 一次只能分配一个值，我们现在却有一整个数组的顶点数据，所以这里我们需要 `gl.vertexAttribPointer` 这个 api。
